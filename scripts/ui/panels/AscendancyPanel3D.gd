...
func _signature() -> String:
	...
	return str(state_ref.get("class_id")) + "|" + str(state_ref.get("selected_ascendancy_id")) + "|" + str(state_ref.get("ascendancy_points")) + "|" + str(allocated_size)
...
func _render(force: bool = false) -> void:
	...
	var header: RichTextLabel = _label("[font_size=20][color=#c59b4a][b]ASCENDANCY[/b][/color][/font_size]\n" + AscSystemScript.summary_text(state_ref), 14)
	...
	for asc_id: String in AscSystemScript.available_ascendancies(state_ref):
		...
		btn.text = ("✓ " if asc_id == selected else "Choose ") + str(data.get("name", asc_id.capitalize()))
		btn.tooltip_text = str(data.get("description", ""))
		btn.focus_mode = Control.FOCUS_NONE
		btn.disabled = selected != "" and selected != asc_id
		btn.pressed.connect(Callable(self, "_choose").bind(asc_id))
	...
	if selected == "":
		list.add_child(_label("[color=#8f8777]Choose one ascendancy for the current class. It is locked for now to keep the demo honest.[/color]", 13))
		return
...
func _node_row(node: Dictionary) -> Control:
	...
	var text: String = ("✓ " if allocated else ("+ " if can_allocate else "· ")) + str(node.get("name", "Node")) + " — cost " + str(int(node.get("cost", 2)))
